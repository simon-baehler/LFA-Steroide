...
savePath = "/home/mlbd/Desktop/LFA-Projet/LFA-Steroide/logs5sens/senspe";

// Fuzzy system parameters 
fixedVars = false;
nbRules = 5;		// Maximum number of rules
nbMaxVarPerRule = 5;	// Maximum number of vars per rule 
nbOutVars = 5;		// Number of output variables
nbInSets = 3;		// Number of input membership functions (MFs)
nbOutSets = 2;		// Number of output MFs
inVarsCodeSize = 6;	// Bits used to encode the index of an input variab (rule genome)
outVarsCodeSize = 3;	// Bits used to encode the index of an output variable index (rule genome)
inSetsCodeSize = 2;	// Bits used to encode the input MFs
outSetsCodeSize = 1;	// Bits used to encode the output MFs
inSetsPosCodeSize = 7;	// Bits used to encode the values of the input MFs
